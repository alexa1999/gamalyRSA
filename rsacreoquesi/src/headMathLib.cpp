...
bool mathLib::fermatTest(int numTests, ZZ pseudoPrime, int bits) {
	mathLib m;
	RC4 r(bits);
	ZZ a;
	string binario;
	...
	for (int i = 0; i < numTests; i++)
	{
		...
		a = m.modulo(r.convertirBinaryToZZ(binario), (pseudoPrime - 1) + 1);
		if (m.expoM(a, pseudoPrime - 1, pseudoPrime) != 1)
		{
			return false;
		}
	}
